...
//
// 4.0rc4c Aug-2025 (FW_4 compatible reliability pack, backward compatible)
...
//  - NOTE: This build preserves the classic :SBN / :DIR protocol used by the Python uploader.
//
#include <EEPROM.h>
...
// legacy addresses (for migration)
...
const char FW_ver[] = "4.0rc4c";  // FW_4 compatible tag
char HW_ver[10];
...
struct CfgV1 {
  ...
  char     hw[10];  // "vX.Y"
  uint32_t spi_len; // bytes expected
  uint16_t spi_crc; // CRC16-CCITT
  uint8_t  spi_has; // valid flag
  uint16_t cfg_crc; // CRC over struct (except cfg_crc)
};
...
const unsigned long DIR_BYTE_TIMEOUT_MS = 2000;
...
void setup() {
  ...
  pinMode( EN_A11, OUTPUT );   pinMode( EN_A12, OUTPUT );   pinMode( EN_A13, OUTPUT );
  pinMode( EN_A14, OUTPUT );   pinMode( EN_A15, OUTPUT );
  ...
  // Load config or migrate
  if (!read_cfg(cfg)) { migrate_legacy_to_cfg(cfg); write_cfg(cfg); }
  ...
  // autoload (only if SPI CRC/len valid)
  if (cfg.autoLoad) {
...
void migrate_legacy_to_cfg(CfgV1 &out){
  ...
  char hw[10]; EEPROM.get(LEGACY_HW, hw);
  if (hw[0] != 'v') { for (uint8_t i=0;i<10;i++) hw[i] = (i<sizeof(DEFAULT_HW_VER))? DEFAULT_HW_VER[i] : 0; EEPROM.put(LEGACY_HW, hw); }
  if (mem < rom16K || mem >= lastType) mem = rom512K;
  if (save > 1) save = 0; if (aut > 1) aut = 0;

  out.magic=CFG_MAGIC; out.ver=CFG_VER; out.mem=mem; out.saveSPI=save; out.autoLoad=aut;
...
void printhelp(){
  Serial.println(F(
    ...
    ":READSPI <addr> <len>   Stream raw SPI bytes (hex lines)\n"
    ":SBN <start> <len> then payload\n"
    ":DIR <start> <len> then payload\n"
    ":help     This help\n"
  ));
}
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (ch == 0x0A) { // LF -> end of frame (command ready)
      if (rxFrame.length() == 0){ Serial.println(F(">")); rxFrame = ""; return; }
      ...
      // READSPI <addr> <len>  (hex dump 16/line)
      if (rxFrame == ":READSPI") {
        unsigned int addr = Serial.parseInt();
        unsigned int len  = Serial.parseInt();
        // consume trailing CR/LF
        while (Serial.peek() == '\r' || Serial.peek() == '\n') { Serial.read(); }

        unsigned int printed = 0;
        while (len > 0) {
          uint8_t lineCount = (len > 16) ? 16 : len;
          // address header
          Serial.print(F("$")); Serial.print(addr, HEX); Serial.print(F(": "));
          SS_LO; spi_transfer(READ); spi_transfer((byte)(addr>>8)); spi_transfer((byte)(addr));
          for (uint8_t i=0;i<lineCount;i++){
            uint8_t b = spi_transfer(0xFF);
            if (b < 16) Serial.print('0');
            Serial.print(b, HEX);
            if (i != lineCount-1) Serial.print(' ');
          }
          SS_HI;
          Serial.println();
          addr += lineCount; len -= lineCount; printed += lineCount;
        }
        rxFrame = "";
        return;
      }
      ...
      if (rxFrame == ":SBN") {
        ...
        (void)transferred; // best-effort; assume host sent full len
      ...
      if (rxFrame == ":DIR") {
        ...
        while (bytecount > 0) {
          if (Serial.available() > 0) {
            ...
            // avoid hard hang if host dies mid-transfer
            break;
          }
        }
        Serial.println(F(">"));
